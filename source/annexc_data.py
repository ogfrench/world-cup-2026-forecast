# FIFA 2026 World Cup - Annex C of the Competition Regulations.
# All 495 = C(12,8) combinations mapping which 8 groups' third-placed teams
# advance -> which third-placed team fills each of the 8 "winner vs 3rd" slots.
#
# Source: Wikipedia "2026 FIFA World Cup knockout stage" (transcribed FIFA Annex C).
# Row format: "<n>: <8 group letters whose 3rd advances> ; <8 assignments>"
# The 8 assignment columns are, in order, the third-placed team that plays:
#   1A (match 79), 1B (match 85), 1D (match 81), 1E (match 74),
#   1G (match 82), 1I (match 77), 1K (match 87), 1L (match 80)
ANNEX_C_RAW = """
...
# Column order in the table -> R32 match number that the third-placed team fills.
# Columns are: 1A, 1B, 1D, 1E, 1G, 1I, 1K, 1L
COLS_TO_MATCH = [79, 85, 81, 74, 82, 77, 87, 80]


def parse_annex_c():
    """Return dict: frozenset(8 group letters) -> {match_no: third_group_letter}."""
    table = {}
    for line in ANNEX_C_RAW.strip().splitlines():
        line = line.strip()
        ...
        mapping = {}
        for col_i, a in enumerate(assigns):
            g = a.replace('3', '').strip()  # "3E" -> "E"
            mapping[COLS_TO_MATCH[col_i]] = g
        table[key] = mapping
    return table
...
if __name__ == '__main__':
    t = parse_annex_c()
    print('Parsed Annex C rows:', len(t))
    # show one
    k = frozenset(list('EFGHIJKL'))
    print('Example EFGHIJKL ->', t[k])
