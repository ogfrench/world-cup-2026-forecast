...
- Title Odds: each team's chance of winning plus a reach-round matrix, against Opta and the
  bookmakers as pre-tournament comparisons on the Day 0 view, with a Live / Day 0 toggle. The live
  view narrows to the teams still in it; Day 0 shows the full pre-tournament field.
...
- Knockout Phase: the bracket fills in match by match as each group finishes, every tie carrying
  ...
  score as it lands. The final and the third-place play-off are predicted once the semi-finals are
  decided. The live Road to the final narrows to the teams still in it and the rounds still to play.
...
- Group and knockout results are baked into the payload, so the page stays a full, self-contained
  archive after the live feed stops (the sundown cutoff, the day after the final).
