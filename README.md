...
- **Knockout Phase** - the predicted bracket against the real results, filling in match by match as each group finishes: a tie appears the moment both its teams are mathematically determined, then shows an in-play/awaiting badge as it kicks off and the real score as it lands. Predicted scorelines follow the single most likely path the tie takes: the 90-minute result, into extra time if level, to penalties if still level. So a dominant team is picked to win in regulation and a tight tie is shown level with a "120'" marker, decided on penalties.
